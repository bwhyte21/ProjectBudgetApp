...
export const fetchBills = createAsyncThunk("bills/fetch", () =>
  billsApi.list(),
);
export const createBill = createAsyncThunk("bills/create", (input: BillInput) =>
  billsApi.create(input),
);
export const updateBill = createAsyncThunk(
  "bills/update",
  ({ id, input }: { id: string; input: BillInput }) =>
...
const billsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(markBillPaid.fulfilled, (state, action: PayloadAction<Bill>) => {
        const idx = state.items.findIndex((b) => b.id === action.payload.id);
        if (idx !== -1) state.items[idx] = action.payload;
      });
  },
